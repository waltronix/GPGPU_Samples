...
void boxBlurSplit(uchar4* pDataIn, uchar4* pDataOut, int width, int height, int blurSize, int blockDimX, int blockDimY);

namespace CLISamples 
{
    public ref class BoxBlurSplit : BoxBlurBase
    {
    protected:
        virtual array<Byte>^ PerformFilter(array<Byte>^ dataIn, int width, int height) override
        {
            ...
            boxBlurSplit((uchar4*)pinnedDataIn, (uchar4*)pinnedDataIout, width, height, 
